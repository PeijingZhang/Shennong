...
class expiMap(nn.Module, CVAELatentsModelMixin):
    ...
    def __init__(self,
        ...
        if recon_loss == "nb":
            self.theta = torch.nn.Parameter(torch.randn(self.input_dim, self.n_conditions).to(self.device))  ### to(device)
        else:
            self.theta = None
        ...
        self.encoder = ExtEncoder(encoder_layer_sizes,
                                  ...
                                  self.dr_rate,
                                  self.n_conditions,
                                  self.n_ext_encoder).to(self.device)
        ...
        if self.soft_mask:
            ...
            self.mask = mask.t().to(self.device)  ### to(device)
            mask = None
        else:
            self.mask = None
        ...
        if self.soft_ext_mask:
            ...
            self.ext_mask = ext_mask.t().to(self.device)
            ext_mask = None
        else:
            self.ext_mask = None
        ...
        self.decoder = MaskedLinearDecoder(self.latent_dim,
                                           ...
                                           self.n_ext_decoder,
                                           self.n_ext_m_decoder).to(self.device)  ### to(device)
        ...
        if self.use_l_encoder:
            self.l_encoder = ExtEncoder([self.input_dim, 128],
                                        ...
                                        self.use_dr,
                                        self.dr_rate,
                                        self.n_conditions).to(self.device)  ### to(device)
            
        self.to(self.device)  ### to(device)
    ...
    def forward(self, x=None, batch=None, sizefactor=None, labeled=None):
        ...
        if sizefactor is not None:
            sizefactor = sizefactor.to(self.device)
