...
class expiMapTrainer(trVAETrainer):
    ...
    def check_prox_ops(self):
        ...
        use_mask = use_main and hasattr(self.model, 'mask') and self.model.mask is not None
        use_prox_ops['main_soft_mask'] = use_mask and self.alpha_l1 is not None

        use_ext = (hasattr(self.model, 'n_ext_decoder') and 
                  self.model.n_ext_decoder > 0 and 
                  self.gamma_ext is not None)
        use_ext = use_ext and self.model.decoder.L0.ext_L.weight.requires_grad
        use_prox_ops['ext_unannot_l1'] = use_ext

        use_ext_m = (hasattr(self.model, 'n_ext_m_decoder') and 
                    self.model.n_ext_m_decoder > 0 and 
                    self.alpha_l1 is not None)
        use_ext_m = (use_ext_m and 
                    hasattr(self.model.decoder.L0, 'ext_L_m') and 
                    self.model.decoder.L0.ext_L_m.weight.requires_grad)
        use_prox_ops['ext_soft_mask'] = use_ext_m and hasattr(self.model, 'ext_mask') and self.model.ext_mask is not None
    ...
    def init_anneal(self):
        ...
        use_soft_mask = (self.use_prox_ops.get('main_soft_mask', False) or 
                        self.use_prox_ops.get('ext_soft_mask', False))
        ...
        if self.use_prox_ops.get('ext_unannot_l1', False) and self.gamma_epoch_anneal is not None:
    ...
    def apply_prox_ops(self):
        if 'main_soft_mask' in self.prox_ops:
            self.prox_ops['main_soft_mask'](self.model.decoder.L0.expr_L.weight.data.to(self.device))
        if 'main_group_lasso' in self.prox_ops:
            self.prox_ops['main_group_lasso'](self.model.decoder.L0.expr_L.weight.data.to(self.device))
        if 'ext_unannot_l1' in self.prox_ops:
            self.prox_ops['ext_unannot_l1'](self.model.decoder.L0.ext_L.weight.data.to(self.device))
        if 'ext_soft_mask' in self.prox_ops:
            self.prox_ops['ext_soft_mask'](self.model.decoder.L0.ext_L_m.weight.data.to(self.device))
